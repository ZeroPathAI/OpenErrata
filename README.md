...
and its prompts, details, and reasoning behind every investigation are designed
to be auditable and free from bias.
...
If you do not have an OpenAI API key, you can receive the errata generated by
other users, but you can't generate them on demand. Right now the public
instance is configured to investigate the most popular posts every hour (though
this may change as we hit spend thresholds).

The extension connects to the hosted API at `api.openerrata.com` by default —
no configuration needed. To point it at a self-hosted instance, open the
extension options page and change the API URL.
